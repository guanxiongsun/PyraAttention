...
function Trainer:test(epoch, dataloader)
   ...
   for n, sample in dataloader:run(false) do
      ...
      for i = 1, #flippedOut do
         if i <= #flippedOut/2 then
            table.insert(heatOut, flippedOut[i])--热图翻转的结果
         else
      ...
      for i = 1, #output do
         if i <= #output/2 then
            table.insert(originheatOut, output[i])--热图
         else
            table.insert(originmaskOut, output[i])--mask
         end
      end
      output=originheatOut--原始heat
