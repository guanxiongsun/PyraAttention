...
function createModel(opt)
    ...
    for i = 1,opt.nStack do
        ...
        ---------------------------------------------------------------------------
        --用于产生mask的hourglass模块
        local hgmask=hourglass(4,opt.nFeats, opt.nResidual, intermask, inputRes,'preact',B,C)
        ---------------------------------------------------------------------------
        ...
        local llmask = preact(opt.nFeats,hgmask)
        llmask = lin(opt.nFeats, opt.nFeats, llmask)


        -- -- Predicted Masks
        local tmpOutmask = nnlib.SpatialConvolution(opt.nFeats,1,1,1,1,1,0,0)(llmask)
        tmpOutmask= nnlib.Sigmoid()(tmpOutmask)

        table.insert(out_mask,tmpOutmask)
        ...
        if i < opt.nStack then
            ...
            ---------------------------------------------------------------------------
            -- -- use mask4select to select inter
            -- -- Use Mask to select Featuremap->3x3 conv to 256; replicate to 256
            local mask4select = nnlib.SpatialConvolution(1,opt.nFeats,3,3,1,1,1,1)(tmpOutmask)
            mask4select=nnlib.ReLU(true)(mask4select)
            inter = nn.CMulTable()({inter, mask4select})
            ----------------------------------------------------------------------------

            local llmask_ = nnlib.SpatialConvolution(opt.nFeats,opt.nFeats,1,1,1,1,0,0)(llmask)
            local tmpOutmask_ = nnlib.SpatialConvolution(1,opt.nFeats,1,1,1,1,0,0)(tmpOutmask)
            intermask = nn.CAddTable()({intermask, llmask_, tmpOutmask_})
        end
    end
