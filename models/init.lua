...
function M.setup(opt, checkpoint)
   local model
   if checkpoint then---有保存点，则将模型加载到cuda
      local modelPath = paths.concat(opt.resume, checkpoint.modelFile)
      assert(paths.filep(modelPath), 'Saved model not found: ' .. modelPath)
      print('=> Resuming model from ' .. modelPath)
      model = torch.load(modelPath):cuda()---将模型加载到cuda
   elseif opt.retrain ~= 'none' then----通过retrain加载
      assert(paths.filep(opt.retrain), 'File not found: ' .. opt.retrain)
      print('Loading model from file: ' .. opt.retrain)
      model = torch.load(opt.retrain):cuda()
      model.__memoryOptimized = nil
   elseif opt.loadModel ~= 'none' then----通过loadmodel加载
      ...
      model = require('models/' .. opt.netType)(opt)---读取model
   end
